...
export const NetflixLandingComponent = () => {
  ...
  const [filteredTrendingMovies, setFilteredTrendingMovies] = useState<any[]>(
    []
  );
  const [filteredRatedMovies, setFilteredRatedMovies] = useState<any[]>([]);
  const [filteredPopularSeries, setFilteredPopularSeries] = useState<any[]>([]);
  const [filteredTrendingSeries, setFilteredTrendingSeries] = useState<any[]>(
    []
  );
  const [filteredRatedSeries, setFilteredRatedSeries] = useState<any[]>([]);
  ...
  const selectedMovie =
    selectedMovieId && allMovies.length > 0
      ? allMovies.find((movie: any) => movie.id === selectedMovieId)
      : null;

  const selectedSeries =
    selectedSeriesId && allSeries.length > 0
      ? allSeries.find((series: any) => series.id === selectedSeriesId)
      : null;
  ...
  useEffect(() => {
    if (filter.length > 0) {
      const filteredPopularMovies = (movies?.results || []).filter(
        (movie: any) => movie.title.toLowerCase().includes(filter.toLowerCase())
      );
    ...
    if (filter.length > 0) {
      const filteredRatedMovies = (ratedMovies?.results || []).filter(
        (movie: any) => movie.title.toLowerCase().includes(filter.toLowerCase())
      );
    ...
    if (filter.length > 0) {
      const filteredPopularSeries = (popularSeries?.results || []).filter(
        (series: any) =>
          series.name.toLowerCase().includes(filter.toLowerCase())
      );
